...
const UserDetail: React.FC = () => {
  const { id } = useParams<{ id: string | number }>(); // Extract ID from URL parameter
  const { getUserById, customers } = useAppContext();
  ...
  const handleToggleInfo = (infoType: string) => {
    setSelectedInfo((prev) => (prev === infoType ? null : infoType));
  };
  if (isLoading) {
    ...
    <div className="user-detail">
      ...
      <div className="user_black">
        ...
        <div className="blacklisted">
          <button className={`status-${customer.status}`}>{customer.status} User</button>
      ...
      <div className="user_content-info">
        <div className="user-content">
          <div className="user_name">
            ...
            <p className={`status-${customer.status}`}>{customer.salt}</p>
          </div>
        ...
        <div className="need-inf">
          {["Account Details", "Document", "Bank Details", "Loans", "Savings", "App and system"].map((infoType) => (
            <p key={infoType} onClick={() => handleToggleInfo(infoType)} className={selectedInfo === infoType ?'active' : '' }>
              {infoType}
            </p>
          ))}
        </div>
       ...
       {selectedInfo === "Account Details" && (
         ...
         <h4>Personal Information</h4>
           <div className="cutome-name">
             ...
             <div className="name">
               <p>Marital status</p>
               <strong>{customer.marital} widow</strong>
             </div>
             <div className="name">
               <p>children</p>
           ...
           <div className="socials">
            ...
            <div className="social-content">
              <p>Twitter</p>
              <strong>@{customer.name.first}_{customer.name.last}</strong>
            </div>
            <div className="social-content">
              <p>FaceBook</p>
              <strong>{customer.name.first} {customer.name.last}</strong>
