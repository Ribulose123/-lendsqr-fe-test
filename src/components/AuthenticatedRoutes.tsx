...
const Customers: React.FC = () => {
  ...
  const handleCustomerClick = (customer: Customer) => {
    ...
    // Check if `id` is an object and try to access `value` property if it exists
    if (typeof customer.id === 'object' && customer.id !== null) {
      // Log the object to see its structure
      console.log("Customer ID is an object:", customer.id);
  
      // Access 'value' or another property if applicable
      customerId = customer.id.value || customer.id.id || customer.id.toString();
    } else {
      // If `id` is a primitive, assign it directly
      customerId = customer.id;
    }
  
    // Ensure the ID is valid and log it
        ...
        <tbody>
          {currentCustomers.map((customer, index) => (
            <tr key={customer.id || customer.email || index}>
              ...
              <td>
              <Link to={`/users/${customer.id}`} onClick={(e) => { 
                  e.preventDefault();
